...
### Warning 4: Measure [Sales Volume] is missing a DIVIDE guard

**Area:** DAX Measures
**Object:** Measure: Sales Volume (in table _Measures)
**Detail:**
The measure `[Sales Volume]` includes a calculation `[Returned Items] / [Total Units Sold]` which is not wrapped in `DIVIDE`. If `[Total Units Sold]` is zero or blank in any filter context, this will return a DAX error value that propagates to visuals.

**Suggested action:**
Replace with `DIVIDE([Returned Items], [Total Units Sold], BLANK())`.
...
5. Wrap `[Sales Volume]` division in `DIVIDE`
